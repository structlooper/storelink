                ...
                <table class="table table-condensed">
                    ...
                    <tbody>
                        ...
                        foreach ($rows as $row) {
                            ...
                            echo '<tr><td colspan="5" class="no-border">#' . $r . ': &nbsp;&nbsp;' . product_name($row->product_name, ($printer ? $printer->char_per_line : null)) . ($row->variant ? ' (' . $row->variant . ')' : '') , ($row->serial_no ? '<br>' . $row->serial_no : '') . '<span class="">' . ($row->tax_code ? '  *' . $row->tax_code : '') . '</span></td></tr>';
                            if (!empty($row->second_name)) {
                                echo '<tr><td colspan="5" class="no-border">' . $row->second_name . '</td></tr>';
                            }
//                            echo '<tr><td class="no-border border-bottom">' . $this->sma->formatQuantity($row->unit_quantity) . ' x ' . ($row->item_discount != 0 ? '(' . $this->sma->formatMoney($row->unit_price + $row->item_discount) . ' - ' . $this->sma->formatMoney($row->item_discount) . ')' : $this->sma->formatMoney($row->unit_price)) . ($row->item_tax != 0 ? ' [' . lang('tax') . ' <small>(' . ($Settings->indian_gst ? $row->tax : $row->tax_code) . ')</small> ' . $this->sma->formatMoney($row->item_tax) . ($row->hsn_code ? ' (' . lang($row->product_type == 'service' ? 'sac_code' : 'hsn_code') . ': ' . $row->hsn_code . ')' : '') . ']' : '') . '</td><td class="no-border border-bottom text-right">' . $this->sma->formatMoney($row->subtotal) . '</td></tr>';
                            echo '<tr><td class="no-border border-bottom">' . $this->sma->formatMoney($product_basic_details['cost']) . ' </td><td>'. number_format((float)((doubleval($product_basic_details['cost']) - doubleval($product_basic_details['price'])) * 100)/doubleval($product_basic_details['cost']) , 2, '.', '').'%</td><td> ' .$this->sma->formatMoney($product_basic_details['price']). '</td><td>'.$this->sma->formatQuantity($row->unit_quantity).'</td><td>'. ($row->item_tax != 0 ? ' [' . lang('tax') . ' <small>(' . ($Settings->indian_gst ? $row->tax : $row->tax_code) . ')</small> ' . $this->sma->formatMoney($row->item_tax) . ($row->hsn_code ? ' (' . lang($row->product_type == 'service' ? 'sac_code' : 'hsn_code') . ': ' . $row->hsn_code . ')' : '') . ']' : $this->sma->formatMoney(0)) . '</td><td class="no-border border-bottom text-right">' . $this->sma->formatMoney($row->subtotal) . '</td></tr>';
            ...
            <div class="order_barcodes text-center">
                ...
                <br>
                <?php // $this->sma->qrcode('link', urlencode(admin_url('sales/view/' . $inv->id)), 2); ?>
            </div>
            <div style="clear:both;"></div>
        </div>
