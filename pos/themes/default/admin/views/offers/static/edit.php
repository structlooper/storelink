...
<?php
if(@isset($offer_products) and sizeof($offer_products) > 0) {
    $value = ' ';
    foreach ($offer_products as $prod_1) {
        $value = $value . '_' . strval($prod_1['product_id']);
    }
}
?>
<div id="value_test" style="display: none">
<?= $value?>
</div>
...
<div class="box">
    <div class="box-header">
        <h2 class="blue"><i
                class="fa-fw fa fa-heart-o"></i>Edit static offer Details
        </h2>
    ...
    <div class="box-content">
        <div class="row">
            ...
            <div class="col-lg-6">
                ...
                <?php
                echo admin_form_open('offers/edit_static_offer/' . $offer['offer_id'], 'id="action-form" enctype="multipart/form-data"');?>
                ...
                <div class="form-group">
                    <?php if(@isset($offer_products) and sizeof($offer_products) > 0){
                        $value = ' ';
                        foreach($offer_products as $prod_1){
                            $value =  $value .'_'.strval($prod_1['product_id']);
                        }
                ...
                <div class="form-group">
                    <div class="row">
                        <div class="col-sm-6">
                            ...
                            <input type="hidden" id="products_added" name="offer_products" value="<?=$value ?>" >
                        </div>
                        <div class="col-sm-6 text-right">
                            <button type="button" class="btn btn-primary btn-sm" onclick="add_new_products()">Add product</button>
                        </div>
                    </div>
                    <div id="products_offed"  >
                        <ul class="list-group " style="margin-top:2px;  max-width: 55rem;" id="product-list-group">
                            <?php
                            foreach($offer_products as $prod_1){
                               echo '<li class="list-group-item row_'.$prod_1['product_id'].'"><div class="row" ><div class="col-sm-10" >'.$prod_1['name'].'</div><div class="col-sm-2 text-right"><button type="button" onclick="remove_product('.$prod_1['product_id'].')" class="btn btn-sm btn-danger"><i class="fa fa-trash"></i></button></div></div></li>';
                            }
                            ?>
                        </ul>
                    </div>
                        ...
                        <div class="form-group">
                            <div class="row">
                                <div class="col-sm-6">
                                    ...
                                    <input type="hidden" id="products_added" name="offer_products" value="" >
                                </div>
                                <div class="col-sm-6 text-right">
                                    <button type="button" class="btn btn-primary btn-sm" onclick="add_new_products()">Add product</button>
                                </div>
